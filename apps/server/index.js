...
import { EXPIRED_TIME, getPriceTokens, PRICE_TOKENS } from './constants';
    ...
    try {
        ...
        const getVolumeWithPrice = (volumeObj = {}) => {
            return Object.entries(volumeObj).reduce((preResult, [tokenId, amount]) => {
                return BigNumber(preResult).plus(
                    BigNumber(revertPrecision(amount)).multipliedBy(getPriceTokens(tokenId))
                ).toNumber()
            }, 0)
        }
        ...
        res.json({
            ...
            data: dataPools.map((pool) => {
                ...
                const priceTokenA = getPriceTokens(tokenAId)
                const priceTokenB = getPriceTokens(tokenBId)
                const poolKey = PoolKey.fromTokenPair(
                    TokenPair.from(TokenId.from(tokenAId), TokenId.from(tokenBId))
                ).toBase58()
...
app.get('/pool/info/:key', async (req, res) => {
    ...
    try {
        ...
        const volumes1D = getVolumePool(dataPoolsAction1D)
        const tvl = BigNumber(revertPrecision(tokenAAmount)).multipliedBy(getPriceTokens(tokenAId))
            .plus(BigNumber(revertPrecision(tokenBAmount)).multipliedBy(getPriceTokens(tokenBId))).toNumber()
        res.status(200)
        res.json({
            error: false,
            data: {
    ...
    try {
        ...
        res.json({
            error: false, data: Object.entries(result).map(([tokenId, info]) => ({
                ...
                volume: {
                    usd: BigNumber(revertPrecision(volumes[tokenId]?.volume) || 0).multipliedBy(getPriceTokens(tokenId)).toNumber()
                },
                fdv: {
                    usd: BigNumber(revertPrecision(info?.total_amount) || 0).multipliedBy(getPriceTokens(tokenId)).toNumber()
                },
                price: {
                    usd: getPriceTokens(tokenId)
                },
                tvl: {
    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                fdv: {
                    usd: BigNumber(revertPrecision(info?.total_amount) || 0).multipliedBy(getPriceTokens(id)).toNumber()
                },
                volume_1d: {
                    usd: BigNumber(revertPrecision(volumes[id]?.volume) || 0).multipliedBy(getPriceTokens(id)).toNumber()
                },
                price: {
                    usd: getPriceTokens(id)
                },
                tvl: {
                    usd: BigNumber(revertPrecision(info?.total_amount) || 0).toNumber()
                }
