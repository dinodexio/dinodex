...
export function ModalPreviewPool({
  dataPool,
  onClickAddPool,
}: modalPreviewPoolProps) {
  return (
    <div className="h-max w-full">
      <div className="relative flex items-center justify-center ">
        <span className="text-[24px] font-[600] text-black">Add Liquidity</span>
        <DialogClose className="absolute right-0 top-[-4px]">
          <svg
      ...
      <div className="mb-[15px] mt-[26px] flex flex-col gap-[15px]">
        <div className={styles["content-modal-preview-header"]}>
          <div className={styles["content-modal-preview-header-token"]}>
            ...
            <span className={styles["preview-header-text"]}>
              {dataPool?.tokenPool?.first?.symbol}/
              {dataPool?.tokenPool?.second?.symbol}
            </span>
          </div>
          <div className={styles["content-modal-preview-header-status"]}>
            <span className={styles["preview-header-text"]}>In Range</span>
            <div
              className={`${styles["preview-header-status-info"]} ${styles["status-in-range"]}`}
            ></div>
          </div>
        </div>
        <div className={styles["content-preview-pool-token"]}>
          ...
          <div className={styles["preview-pool-token-item"]}>
            ...
            <span className="preview-header-fee-tier-text">
              {dataPool?.feeTier}%
            </span>
          </div>
        </div>
        <div className={styles["selected-range-content"]}>
          ...
          <div className={styles["switch-select-range"]}>
            <div
              className={`${styles["switch-select-range-item"]} ${styles["switch-select-range-item-active"]}`}
            >
              {dataPool?.tokenPool?.first?.symbol}
            </div>
            <div className={`${styles["switch-select-range-item"]}`}>
              {dataPool?.tokenPool?.second?.symbol}
            </div>
          </div>
        </div>
        <div className={styles["content-min-price"]}>
          <div className={styles["content-min-price-item"]}>
            <span className={styles["content-min-price-item-title"]}>
              Min Price
            </span>
            <span className={styles["header-preview-pool-text"]}>
              {dataPool?.valueMinPrice?.first < 0.001
                ? `< 0.001`
                : dataPool?.valueMinPrice?.first}
            </span>
            <span className={styles["content-min-price-item-title"]}>
              {dataPool?.tokenPool?.first?.symbol} per{" "}
              {dataPool?.tokenPool?.second?.symbol}
            </span>
            <span className={styles["content-min-price-item-desc"]}>
          ...
          <div className={styles["content-min-price-item"]}>
            ...
            <span className={styles["header-preview-pool-text"]}>
              {dataPool?.valueMinPrice?.second < 0.001
                ? `< 0.001`
                : dataPool?.valueMinPrice?.second}
            </span>
            <span className={styles["content-min-price-item-title"]}>
              {dataPool?.tokenPool?.first?.symbol} per{" "}
              {dataPool?.tokenPool?.second?.symbol}
        ...
        <div className="flex w-full flex-col items-center justify-center gap-[10px] rounded-[21px] border border-black px-[10px] py-[12px]">
          <span className={styles["content-min-price-item-title"]}>
            Current Price
          </span>
        ...
        <div
          className={`${stylesButton["button-swap"]} ${stylesButton["btn-modal-preview-pool"]}`}
          onClick={() => onClickAddPool && onClickAddPool()}
        >
          <span>Add</span>
        </div>
      </div>
