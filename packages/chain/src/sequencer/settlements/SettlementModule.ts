...
import { BlockProverPublicOutput } from "@proto-kit/protocol";
import { BATCH_SIGNATURE_PREFIX, SettlementSmartContractBase, SettlementSmartContract } from "../../protocol/settlement/contracts/SettlementSmartContract";
import { DispatchSmartContract } from "../../protocol/settlement/contracts/DispatchSmartContract";
...
import {
  ...
  type MessageStorage, MinaBaseLayer, Batch, SettleableBatch, Settlement,
  type IncomingMessageAdapter, MinaTransactionSender
...
import type { FeeStrategy } from "@proto-kit/sequencer/dist/protocol/baselayer/fees/FeeStrategy";

import { SettlementUtils } from "./utils/SettlementUtils";
import { BridgingModule } from "./BridgingModule";
import { LazyBlockProof } from "../../protocol/settlement/contracts/SettlementSmartContract";
...
export class SettlementModule
  ...
  public async deploy(
    ...
    const StateTransitionProver = this.protocol.resolve("StateTransitionProver");
    const StateTransitionProverProgram = StateTransitionProver.zkProgrammable.zkProgramFactory()[0];
    const StateTransitionProverProgramCompile = await StateTransitionProverProgram.compile();
    // await StateTransitionProver.zkProgrammable.zkProgram[0].compile();
    console.log("StateTransitionProver", StateTransitionProverProgram, StateTransitionProverProgramCompile);


    const blockProver = this.protocol.resolve("BlockProver");
    const blockProverProgram = blockProver.zkProgrammable.zkProgramFactory()[0];
    const blockProverProgramCompile = await blockProverProgram.compile();
    console.log("blockProverProgram", blockProverProgram, blockProverProgramCompile);

    // LazyBlockProof.tag = blockProverProgram.Proof.tag;
    const settlementClass = sm.getContractClasses();
    await settlementClass.settlement.compile();
    await settlementClass.dispatch.compile();
